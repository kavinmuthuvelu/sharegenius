...
function runBacktest() {
  ...
  if (source === 'dhan') {
    ...
    url = \`/api/backtest/run?token=\${encodeURIComponent(token)}&clientId=\${encodeURIComponent(clientId)}&capital=\${capital}&tradeSize=\${tradeSize}&fromDate=\${fromDate}&toDate=\${toDate}\`;
  } else {
    url = \`/api/backtest/bhavcopy?capital=\${capital}&tradeSize=\${tradeSize}&fromDate=\${fromDate}&toDate=\${toDate}\`;
  }
...
app.get('/api/backtest/bhavcopy', async (req, res) => {
  ...
  for (const sym of symSet) {
    ...
    const trades = runStrategySimulation(sym, rows, parseFloat(tradeSize), fromDate);
    allTrades.push(...trades);
  }
