...
function renderPositions() {
 ...
 tbody.innerHTML = open.flatMap(pos => {
  const cmp = pos.cmp ?? null;
  ...
  const targetPct = TARGET_PCTS[Math.min(pos.averageCount || 0, 3)];
  const targetPrice = pos.avgPrice * (1 + targetPct / 100);
  const progressPct = cmp !== null ? Math.min((cmp / targetPrice) * 100, 100) : 0;
  const avgCount = (pos.averageCount || 0);
  const maxAvgs = 3;
  const targetHit = cmp !== null && cmp >= targetPrice;

  // Generate one row per buy transaction
  return pos.buys.map((buy, idx) => {
    const tradeLabel = `${pos.id}_${idx}`;  // e.g., '1234567890_0', '1234567890_1'
    return `
      <tr ${targetHit ? 'class="alert-row"' : ''}>
        <td>
          <div class="symbol-cell">
            <span class="sym-name">${pos.symbol} <small style="font-weight: normal; color: var(--text3);">${tradeLabel}</small></span>
            ${pos.notes ? `<span class="sym-sub">${pos.notes.slice(0,20)}...</span>` : ''}
          </div>
        </td>
        <td class="num">${fmtCurr(buy.price)}</td>  <!-- This buy's price -->
        <td class="num">${fmt(buy.qty, 0)}</td>
        <td class="num">${fmtCurr(pos.avgPrice * pos.qty, 0)}</td>  <!-- Position total invested -->
        <td class="num">${cmp !== null ? fmtCurr(cmp) : '<span style="color: var(--text3)">Click refresh</span>'}</td>
        ...
        <td>
          <div class="target-bar">
            <div class="target-bar-fill" style="width: ${progressPct}%"></div>
          </div>
        ...
        <td style="text-align: center;">
          ${avgCount === 0 ? '' : `<span class="badge badge-open">${avgCount}/3</span>`}
          ${avgCount === 0 ? '<span class="badge badge-closed"></span>' : ''}
        ...
        <td class="actions" style="display: flex; gap: 4px; justify-content: flex-end; flex-wrap: wrap;">
          ...
          <button class="btn btn-sm btn-red" onclick="deletePosition('${pos.id}')">Delete</button>
        </td>
      </tr>
    `;
  });
