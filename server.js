      ...
      <div id="bt-summary" style="display:none;">
        ...
        <div class="section-header" style="margin-bottom:12px;">
          ...
          <div class="section-actions">
            ...
            <button class="btn btn-green btn-sm" onclick="downloadTradeLogExcel()" title="Download as Excel">
              ⬇ Excel
            </button>
          </div>
        </div>
...
function switchBtUniverse(u) {
  ...
  document.getElementById(u === 'NIFTY50' ? 'bt-uni-n50' : 'bt-uni-n100').classList.add('active');
}
...
function renderBtTrades() {
  ...
  for (const t of trades) {
    ...
    const resultBadge = t.exit_reason === 'TARGET'
      ? '<span class="badge badge-hit">🎯 TARGET</span>'
      : isOpen
      ? '<span class="badge badge-open">📂 OPEN</span>'
      : '<span class="badge badge-closed">EXIT</span>';

    const avgCount  = t.avg_count || 0;
    const totalLegs = 1 + avgCount; // Buy_1 + Avg_0..Avg_(n-1)

    // ── Summary row (yellow highlight) ──────────────────────────
    rows.push(`
      <tr style="background:rgba(245,166,35,0.10);border-top:2px solid rgba(245,166,35,0.4);">
        <td rowspan="${totalLegs + 1}" style="vertical-align:top;padding-top:10px;">
          <span class="sym-name">${t.symbol}</span>
        </td>
        ...
        <td class="num" style="color:var(--gold);font-weight:700;">${fmtCurr(t.entry_price)}</td>
        <td class="num">${fmtCurr(t.exit_price)}</td>
        <td class="num">${fmtCurr(t.invested, 0)}</td>
        <td class="num">${t.hold_days}d</td>
        <td class="num">${avgCount > 0 ? avgCount + '×' : '—'}</td>
    ...
    // ── Leg rows: Buy_1, then Avg_0, Avg_1 … ────────────────────
    // Build legs from t.buys if available, else synthesise
    const legs = [];

    if (t.buys && t.buys.length) {
      // Server sent full buy history
      t.buys.forEach((b, idx) => {
        legs.push({
          label: idx === 0 ? 'Buy_1' : `Avg_${idx - 1}`,
          price: b.price,
          qty:   b.qty,
        });
      });
    } else {
      // Fallback: only entry price available, pad remaining avg legs as "—"
      legs.push({ label: 'Buy_1', price: t.entry_price, qty: null });
      for (let k = 0; k < avgCount; k++) {
        legs.push({ label: `Avg_${k}`, price: null, qty: null });
      }
    }

    // Fill up to totalLegs (in case mismatch)
    while (legs.length < totalLegs) {
      legs.push({ label: `Avg_${legs.length - 1}`, price: null, qty: null });
    }

    for (const leg of legs) {
      const priceCell = leg.price != null
        ? `<span style="background:rgba(245,166,35,0.15);color:var(--gold);padding:2px 8px;border-radius:4px;font-size:11px;font-family:var(--mono);">Actual buy price ${fmtCurr(leg.price)}</span>`
        : `<span style="background:rgba(245,166,35,0.10);color:var(--text3);padding:2px 8px;border-radius:4px;font-size:11px;font-family:var(--mono);">Actual buy price —</span>`;

      rows.push(`
        <tr style="background:rgba(245,166,35,0.04);">
          <td style="color:var(--text2);font-size:11px;font-family:var(--mono);padding-left:18px;">${leg.label}</td>
          <td colspan="14" style="padding:5px 14px;">${priceCell}</td>
        </tr>`);
    }

    // Spacer between trades
    rows.push(`<tr style="height:6px;background:var(--bg);"><td colspan="16"></td></tr>`);
  }

  document.getElementById('bt-trade-tbody').innerHTML = rows.join('');
}
...
function _doExcelExport(trades) {
  ...
  const wins    = closed.filter(t => t.pnl > 0);
  const totalPnl = allT.reduce((s, t) => s + t.pnl, 0);
...
// ── Capital tracking: walk all closed trades chronologically, compute
//    running capital after each exit, find minimum point ─────────────────────
function attachCapitalTracking(allTrades, startCapital) {
  ...
  for (const t of closed) {
    ...
    if (running < minCap) { minCap = running; minDate = t.exit_date; }
  }
  for (const t of allTrades) {
    if (t.exit_reason === 'OPEN') t.capital_after = null;
  }
  return { minCapital: +minCap.toFixed(2), minCapitalDate: minDate };
}
